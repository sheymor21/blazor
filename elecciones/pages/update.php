...
<body>
  ...
  ?>
  <div class="estilo">
    <form action="../Logic/updateRedirect.php?id =<?php echo $_POST['id']; ?>&&que=<?php echo $_GET['id']; ?>" method="post">
      <label for="">Que desea editar?</label>
      <?php $logic->listButtons($_GET['id'], 'editar'); ?>
      <label for="">Coloca id de lo que desea editar</label>
      <div class="row">
        <div class="col">
          <input type="text" class="form-control" id="" aria-describedby="helpId" placeholder="" name="id">
        </div>
        <div class="col">
          <button type="submit" name="" id="" class="btn btn-warning">Editar</button>
        </div>
      </div>
    </form>
  </div>
        ...
        <tbody>
          <?php foreach ($listPartidos as $value) : ?>
            <tr>
              ...
              <td><img src="<?php echo $value->logo;?>" alt=""></td>
